...
export async function middleware(request: NextRequest) {
  // Check if user is authenticated and has Governance Registrar role
  try {
    ...
    if (session.role !== 'governance-registrar') {
    ...
    console.error('Governance Registrar middleware error:', error)
    return NextResponse.redirect(new URL('/login', request.url))
  }
}

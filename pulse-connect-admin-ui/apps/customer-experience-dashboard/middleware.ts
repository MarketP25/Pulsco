...
export async function middleware(request: NextRequest) {
  // Check if user is authenticated and has Customer Experience role
  try {
    ...
    if (session.role !== 'customer-experience') {
    ...
    console.error('Customer Experience middleware error:', error)
    return NextResponse.redirect(new URL('/login', request.url))
  }
}

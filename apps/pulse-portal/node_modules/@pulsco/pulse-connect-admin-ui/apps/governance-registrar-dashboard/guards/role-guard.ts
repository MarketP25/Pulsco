// Governance Registrar Dashboard Role Guard
// Ensures only Governance Registrar role can access the Governance Registrar dashboard
...
export class GovernanceRegistrarRoleGuard {
  ...
  private validateRole(userRole: AdminRoleType): boolean {
    return userRole === 'governance-registrar'
  }
  ...
  private async validateGovernanceApproval(session: UserSession): Promise<boolean> {
    const approval = await this.config.marpClient.getGovernanceApproval({
      ...
      resource: 'governance-registrar-dashboard',
      action: 'access'
    })
  ...
  private async logAccessAttempt(session: UserSession, success: boolean, error?: any): Promise<void> {
    await this.config.marpClient.logAuditEvent({
      ...
      actionSubtype: 'governance_registrar_dashboard',
      userId: session.userId,
...
export default GovernanceRegistrarRoleGuard
